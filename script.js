...
function animateText(){
    if (isTyping) {
        ...
        if (index>1){
            ...
            currentColor=currentColor===(document.documentElement.classList.contains ('light') ? 'black' : '#fff') ? '#23497e' : (document.documentElement.classList.contains('light') ? 'black' : '#fff');
...
// Função para inciar a transição automática dos slides
...
function checkAboutVisibility(){
    const rect=aboutSection.getBoundingClientReact();
